<template>
  <div class="main-box home-goods-detail-container">
    <el-row :gutter="40">
      <el-col :span="16">
        <div style="padding: 15px;border-radius: 10px;box-shadow: 0 0px 6px 2px rgba(34, 39, 54, 0.15);background: white">
          <el-image :src="nowImage" :fit="'contain'" class="goods-image"></el-image>
        </div>
        <div style="margin-top: 20px;overflow-x: auto">
          <div v-for="item in info.picture" class="goods-image-thumb-box" @click="changeImage(item)">
            <el-image :src="sourceUrl+item" :fit="'contain'" class="goods-image-thumb"></el-image>
          </div>
        </div>
      </el-col>
      <el-col :span="8">
        <div style="box-shadow: 0 0px 6px 2px rgba(34, 39, 54, 0.15);border-radius: 8px;overflow: hidden;padding:30px 20px;background: white">
          <div style="font-weight: bold;font-size: 18px">{{info.name}}</div>
          <div style="color: #ff5959;margin: 20px 0;font-size: 18px;font-weight: bold">${{info.price}}</div>
          <div>
            <!--<el-tag effect="dark">{{info.level}} 成新</el-tag>-->
            <el-tag effect="dark" type="success">{{info.free_shipping==1?'free shipping':'not free shipping'}}</el-tag>
            <el-tag effect="dark" type="info">{{info.buy_time}} Buy</el-tag>
            <el-tag effect="dark" type="warning">{{info.view}} view</el-tag>
          </div>
        ...
        <div class="goods-button">
          <el-button round  @click="handleFavourite" type="success">Add to Favorite</el-button>
          <el-button round   @click="handleCart" type="success">Add to Cart</el-button>
          <el-button round  type="primary" @click="handleBuy">Buy</el-button>
        </div>
      </el-col>
    </el-row>
    <el-tabs v-model="activeName" style="margin-top: 20px;margin-bottom: 100px">

      <el-tab-pane label="Description" name="first">
        <div style="background: white;border:1px solid #ccc;padding: 20px;min-height: 200px">
          {{info.desc}}
        </div>
      ...
      <el-tab-pane :label="'Comment('+listTotal+')'" name="second">
        <el-button   @click="handleComment" type="primary" size="small">Comment</el-button>
          <div  style="background: white;border:1px solid #ccc;padding: 20px;margin-top: 15px" v-if="list.length">
...
<script>
  ...
  export default {
    ...
    components: {
      ...
      Order: () => import('@/components/Order/Index'),
    },
    methods: {
      toUser(id){
        this.$router.push({
