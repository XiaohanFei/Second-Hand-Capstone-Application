          ...
          <div>
            <div v-for="item in list" :key="this" style="padding: 20px;border-bottom: 1px solid #ccc">
              ...
              <div style="display: flex;flex-direction: row;align-items: center;margin-top: 10px;color: #999;font-size: 15px">
                <img :src="sourceUrl+item.avatar" style="width: 20px;height: 20px;border-radius: 50%;cursor: pointer" @click="toUser(item.user_id)">
                <div style="margin: 0 10px;"><span style="color: var(--blue);cursor: pointer" @click="toUser(item.user_id)">{{item.name}}</span></div>
                <!--<div>{{item.create_time}}</div>-->
                <div style="margin: 0 10px;"><i class="el-icon-s-comment"></i>{{item.reply}} reply</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      handleAdd(){
        this.$prompt('Say something', 'Tip', {
        }).then(({ value }) => {
          if(value.length){
            addChat({content:value,pid:0}).then(()=>{
              this.content = ''
              this.getList()
            }).catch(()=>{return})
          }else{
